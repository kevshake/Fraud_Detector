...
public class PrometheusMetricsService {
    ...
    private final Counter transactionByMerchantCounter;
    private final Counter transactionByRiskLevelCounter;
    private final Timer transactionProcessingTime;
    private final Gauge transactionQueueSize;
    ...
    private final Gauge amlActiveCases;
    ...
    private final Gauge complianceOpenCases;
    private final Gauge compliancePendingSars;
    ...
    private final Gauge databaseConnectionPoolActive;
    private final AtomicLong activeConnectionsValue = new AtomicLong(0);

    // System Health Metrics
    private final Gauge systemUptime;
    private final Gauge jvmMemoryUsed;
    private final Gauge jvmMemoryMax;
    private final Gauge jvmThreadsActive;
    ...
    private final Gauge modelAuc;
    private final Gauge modelPrecisionAt100;
    private final Gauge modelDriftScore;
    private final AtomicLong modelAucValue = new AtomicLong(0);
    private final AtomicLong modelPrecisionAt100Value = new AtomicLong(0);
    private final AtomicLong modelDriftScoreValue = new AtomicLong(0);
    ...
    private final Gauge screeningQueueSize;
    private final AtomicLong screeningQueueSizeValue = new AtomicLong(0);
    ...
    public PrometheusMetricsService(
        ...
        this.transactionByMerchantCounter = Counter.builder("aml.transactions.by.merchant")
                .description("Transactions by merchant ID")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.transactionByRiskLevelCounter = Counter.builder("aml.transactions.by.risk.level")
                .description("Transactions by risk level")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);
        ...
        this.transactionQueueSize = Gauge.builder("aml.transactions.queue.size", transactionQueueSizeValue, AtomicLong::get)
        ...
        this.amlActiveCases = Gauge.builder("aml.active.cases", amlActiveCasesValue, AtomicLong::get)
        ...
        this.complianceOpenCases = Gauge.builder("compliance.open.cases", complianceOpenCasesValue, AtomicLong::get)
                .description("Number of open compliance cases")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.compliancePendingSars = Gauge.builder("compliance.pending.sars", compliancePendingSarsValue, AtomicLong::get)
        ...
        this.activeConnections = Gauge.builder("system.active.connections", activeConnectionsValue, AtomicLong::get)
                .description("Number of active connections")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.databaseConnectionPoolSize = Gauge.builder("database.connection.pool.size", () -> 0.0)
                .description("Database connection pool size")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.databaseConnectionPoolActive = Gauge.builder("database.connection.pool.active", () -> 0.0)
                .description("Active database connections")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        // Initialize System Health Metrics
        this.systemUptime = Gauge.builder("system.uptime.seconds", () -> (System.currentTimeMillis() - startTime) / 1000.0)
                .description("System uptime in seconds")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.jvmMemoryUsed = Gauge.builder("jvm.memory.used.bytes", () -> (double)(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()))
                .description("JVM memory used in bytes")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.jvmMemoryMax = Gauge.builder("jvm.memory.max.bytes", () -> (double)Runtime.getRuntime().maxMemory())
                .description("JVM memory max in bytes")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.jvmThreadsActive = Gauge.builder("jvm.threads.active", () -> (double)Thread.activeCount())
        ...
        this.modelAuc = Gauge.builder("model.auc", modelAucValue, v -> v.get() / 10000.0)
                .description("Model AUC (Area Under Curve)")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.modelPrecisionAt100 = Gauge.builder("model.precision.at.100", modelPrecisionAt100Value, v -> v.get() / 10000.0)
                .description("Model precision at top 100")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);

        this.modelDriftScore = Gauge.builder("model.drift.score", modelDriftScoreValue, v -> v.get() / 10000.0)
        ...
        this.screeningQueueSize = Gauge.builder("screening.queue.size", screeningQueueSizeValue, AtomicLong::get)
                .description("Current screening queue size")
                .tag("application", "aml-fraud-detector")
                .register(meterRegistry);
    }
    ...
    public void incrementComplianceCaseCreated(CaseStatus status, String priority) {
        ...
        Counter.builder("compliance.cases.created")
            ...
            .register(meterRegistry)
            .increment();
        complianceCaseByStatusCounter.increment();
    ...
    public void incrementComplianceCaseByPriority(String priority) {
        complianceCaseByPriorityCounter.increment();
    ...
    public void setModelAuc(double auc) {
        modelAucValue.set((long) (auc * 10000)); // Store as integer (multiply by 10000 for precision)
    }

    private double getModelAuc() {
        return modelAucValue.get() / 10000.0;
    }

    public void setModelPrecisionAt100(double precision) {
        modelPrecisionAt100Value.set((long) (precision * 10000));
    }

    private double getModelPrecisionAt100() {
        return modelPrecisionAt100Value.get() / 10000.0;
    }

    public void setModelDriftScore(double drift) {
        modelDriftScoreValue.set((long) (drift * 10000));
    }
    ...
    public void setScreeningQueueSize(long size) {
        screeningQueueSizeValue.set(size);
    }
