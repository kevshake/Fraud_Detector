...
public class DroolsRulesService {
    ...
    @Autowired
    public DroolsRulesService(@Autowired(required = false) AerospikeGraphCacheService aerospikeCache) {
        this.aerospikeCache = aerospikeCache;
    }

    @PostConstruct
    public void initializeRules() {
        logger.info("Initializing Drools Rules Engine...");
        try {
            KieServices kieServices = KieServices.Factory.get();
            KieFileSystem kfs = kieServices.newKieFileSystem();

            // Try to load DRL from classpath
            try {
                ...
                if (kieBuilder.getResults().hasMessages(org.kie.api.builder.Message.Level.ERROR)) {
                    ...
                    droolsEnabled = true;
                    logger.info("Drools Rules Engine initialized successfully from DRL files");
                }
            } catch (Exception e) {
                logger.warn("DRL file not found or invalid, using programmatic rules: {}", e.getMessage());
                droolsEnabled = false;
            }
    ...
    private int evaluateProgrammaticRules(TransactionFact fact) {
        ...
        // Rule 4: High ML Score - Block if score > 0.9
        if (fact.getMlScore() > 0.9) {
            fact.setDecision("BLOCK");
            fact.addTriggeredRule("ML_SCORE_HIGH_RISK");
            fact.addReason("ML risk score exceeds 0.9 threshold: " + String.format("%.3f", fact.getMlScore()));
            rulesTriggered++;
        }

        // Rule 5: Medium ML Score - Hold for review if score > 0.7
        if (fact.getMlScore() > 0.7 && fact.getMlScore() <= 0.9 &&
                !"BLOCK".equals(fact.getDecision())) {
            fact.setDecision("HOLD");
            fact.addTriggeredRule("ML_SCORE_MEDIUM_RISK");
            fact.addReason("ML risk score requires review: " + String.format("%.3f", fact.getMlScore()));
            rulesTriggered++;
        }

        // Rule 6: High Betweenness Centrality (potential money mule hub)
        if (fact.getBetweenness() > 0.5) {
            fact.setDecision("HOLD");
            fact.addTriggeredRule("HIGH_BETWEENNESS_HUB");
            fact.addReason("Merchant has high betweenness centrality (potential hub): " +
                    String.format("%.3f", fact.getBetweenness()));
            rulesTriggered++;
        }

        // Rule 7: Velocity Breach - Too many transactions in 1 hour
        if (fact.getPanTxnCount1h() > 10) {
            fact.setDecision("HOLD");
        ...
        // Rule 8: High PageRank with High-Value Transaction (influential + large
        // amount)
        if (fact.getPageRank() > 0.8 && fact.isHighValueTransaction()) {
            fact.setSarRequired(true);
            fact.addTriggeredRule("HIGH_INFLUENCE_HIGH_VALUE");
            fact.addReason("High-influence merchant with high-value transaction");
            rulesTriggered++;
        }
