...
import { FileText, Loader2, Search, Upload, X, Eye } from "lucide-react";
...
function DocumentsDialog({ merchantId, merchantName, onClose }: { merchantId: number; merchantName: string; onClose: () => void }) {
  ...
  const { data: documents, isLoading, isError } = useQuery<MerchantDocument[]>({
    ...
    queryFn: () => apiClient.get<MerchantDocument[]>(`merchants/${merchantId}/documents`).catch(() => []),
    enabled: true,
  });

  const closeUpload = () => { setUploadOpen(false); setUploadFile(null); setUploadType("ID_DOCUMENT"); if (fileInputRef.current) fileInputRef.current.value = ""; };
                  ...
                  <div key={doc.id} className={idx > 0 ? "border-t border-white/10" : ""}>
                    <div className="flex items-center gap-3 px-6 py-3">
                      ...
                      <div className="flex shrink-0 items-center gap-1">
                        {url && <button onClick={() => window.open(url, "_blank", "noopener,noreferrer")} className="rounded px-2 py-1 text-xs text-burgundy-400 transition-colors hover:bg-burgundy-700/20">Open</button>}
                        {isUnverified && (
                          <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2">
            <div className="overflow-hidden rounded-xl border border-white/10 bg-[#0f1a2e] shadow-2xl">
              ...
              <div className="space-y-4 px-6 py-4">
                ...
                <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-burgundy-700/50 bg-[#1a2744] px-4 py-6 text-sm text-burgundy-400 transition-colors hover:bg-[#1f3050]">
                  <Upload size={18} />
                  {uploadFile ? "Change file" : "Choose file"}
                  <input ref={fileInputRef} hidden type="file" onChange={(e) => setUploadFile(e.target.files?.[0] ?? null)} />
                </label>
      ...
      <div className="flex justify-end pb-3">
        <div className="relative w-72">
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-[#1a2744] py-2 pl-9 pr-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-burgundy-700"
          />
        </div>
      </div>
