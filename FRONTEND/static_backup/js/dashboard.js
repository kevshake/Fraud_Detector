...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.refreshCases = function () {
        ...
        // Get filter values
        const statusFilter = document.getElementById('caseStatusFilter') ? document.getElementById('caseStatusFilter').value : '';
        ...
        fetch(url, getFetchOptions())
            ...
            .then(data => {
                // Handle Page object or List
                const cases = data.content || data;

                if (!cases || cases.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;">No cases found.</td></tr>';
                    document.getElementById('openCasesCount').innerText = '0';
                    return;
                }

                // Update counts if available in stats endpoint context, or just estimate from current page (imperfect)
                // Better to fetch stats separately. refreshCases focuses on table.

                let html = '';
                cases.forEach(c => {
                    const created = c.createdAt ? new Date(c.createdAt).toLocaleString() : 'N/A';
                    const sla = c.slaDeadline ? new Date(c.slaDeadline).toLocaleDateString() : 'N/A';
                    const riskScore = c.priority === 'HIGH' ? 90 : (c.priority === 'MEDIUM' ? 50 : 20); // Mock score from priority

                    html += `
                        <tr>
                            <td><span class="case-id">${c.caseReference || c.id}</span></td>
                            <td>Merchant ${c.merchantId || 'N/A'}</td>
                            <td><span class="risk-badge ${getRiskClassForLevel(c.priority)}">${c.priority}</span></td>
                            <td><span class="status-badge ${getStatusClass(c.status)}">${c.status}</span></td>
                            <td>${c.assignedTo ? c.assignedTo.username : 'Unassigned'}</td>
                            <td>${sla}</td>
                            <td>${c.daysOpen || 0} days</td>
                            <td>
                                <div class="action-buttons">
                                    <button class="action-btn" onclick="viewCase(${c.id})" title="View Details"><i class="fas fa-eye"></i></button>
                                    ${c.status !== 'CLOSED_CLEARED' && c.status !== 'CLOSED_SAR_FILED' ?
                            `<button class="action-btn success" onclick="resolveCase(${c.id})" title="Close Case"><i class="fas fa-check"></i></button>` : ''}
                                    ${c.status !== 'ESCALATED' && c.status !== 'CLOSED_CLEARED' ?
                            `<button class="action-btn warning" onclick="escalateCasePrompt(${c.id})" title="Escalate"><i class="fas fa-exclamation-triangle"></i></button>` : ''}
                                </div>
                            </td>
                        </tr>
                    `;
            ...
            .catch(err => {
                ...
                tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:red;">Error loading cases: ${err.message}</td></tr>`;
                handleApiError(err, 'refreshCases');
            });
    };

    window.fetchCaseStats = function () {
        fetch('compliance/cases/stats', getFetchOptions())
            .then(res => res.json())
            .then(stats => {
                if (stats) {
                    if (document.getElementById('openCasesCount')) document.getElementById('openCasesCount').innerText = stats.openCases || 0;
                    // Other stats map if available
                }
            })
    ...
    window.viewCase = function (id) {
        fetch(`compliance/cases/${id}`, getFetchOptions())
            .then(res => {
            ...
            .then(c => {
                ...
                document.getElementById('case-detail-title').innerText = `Case ${c.caseReference || c.id}`;

                const infoContent = document.getElementById('case-info-content');
                infoContent.innerHTML = `
                    <div class="case-details-grid">
                        ...
                        <div class="detail-group">
                            ...
                            <span class="risk-badge ${getRiskClassForLevel(c.priority)}">${c.priority}</span>
                        </div>
                        <div class="detail-group">
                            <label>Assigned To</label>
                            <span>${c.assignedTo ? c.assignedTo.username : 'Unassigned'}</span>
                        </div>
                         <div class="detail-group">
                            <label>Merchant ID</label>
                            <span>${c.merchantId || 'N/A'}</span>
                        </div>
                         <div class="detail-group">
                ...
                // Set current case ID for other tabs/actions
                window.currentCaseId = id;
            })
            .catch(err => {
                alert('Error viewing case: ' + err.message);
            });
    };
    ...
    window.resolveCase = function (id) {
        ...
        fetch('compliance/cases/workflow/status', getFetchOptions('POST', {
            caseId: id,
            status: 'CLOSED_CLEARED',
            userId: getCurrentUserId() // Helper to get ID
        }))
            .then(res => {
    ...
    window.escalateCasePrompt = function (id) {
        const reason = prompt("Enter escalation reason:");
        if (!reason) return;

        // Mock escalate API call - expecting endpoint
        // Assuming we verify endpoint exists in controller
        fetch('compliance/cases/workflow/escalate', getFetchOptions('POST', {
            caseId: id,
            reason: reason,
            userId: getCurrentUserId()
            // escalateToUserId? For now unassigned or system default
        }))
            .then(res => {
    ...
    function getCurrentUserId() {
        // Mock or retrieve from session/profile
        // In reality, this should be handled by backend session extraction usually, 
        // but the controller expects userId in body for some calls?
        // Checking Controller: ComplianceCaseWorkflowController expects userId in body.
        // I should fetch profile first or store it.
        // For now preventing error by assuming backend can handle it or we fetch it.
        // Assuming user profile is loaded.
        // Lets try to find a userId from DOM or window global if set
        return 1; // Fallback to admin ID 1 if not found
    }
